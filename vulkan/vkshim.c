...
// ---- fresh-device warm-up -------------------------------------------------
// The blob intermittently LOSES work (draws, copies) submitted in the first
// render submission(s) of a freshly created VkDevice — proven with
// tools/vkquad/vkreinit: first frame after device recreate drops most quads;
// in RetroArch a renderer restart shows missing/duplicated menu icons because
// the icon texture uploads ride those first command buffers. Five sacrificial
...
static void warmup_device(VkPhysicalDevice pd, VkDevice dev,
	...
	F(AllocateMemory); F(BindImageMemory); F(CreateImageView);
	F(CreateRenderPass); F(CreateFramebuffer); F(CreateCommandPool);
	F(AllocateCommandBuffers); F(BeginCommandBuffer); F(CmdBeginRenderPass);
	...
	if (pCreateImage(dev, &imci, NULL, &img) != VK_SUCCESS) goto out;
	{
		VkMemoryRequirements mr;
		pGetImageMemoryRequirements(dev, img, &mr);
		...
		if (pAllocateMemory(dev, &mai, NULL, &mem) != VK_SUCCESS) goto out;
		if (pBindImageMemory(dev, img, mem, 0) != VK_SUCCESS) goto out;
	}
	{
		...
		if (pCreateImageView(dev, &vci, NULL, &view) != VK_SUCCESS) goto out;
	}
	{
		...
		if (pCreateRenderPass(dev, &rpci, NULL, &rp) != VK_SUCCESS) goto out;
	}
	{
		...
		if (pCreateFramebuffer(dev, &fbci, NULL, &fb) != VK_SUCCESS) goto out;
	}
	{
		...
		VkCommandBuffer cmd;
		if (pAllocateCommandBuffers(dev, &cbai, &cmd) != VK_SUCCESS) goto out;
		for (int w = 0; w < WARMUP_ROUNDS; w++) {
			VkCommandBufferBeginInfo bi = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
			if (pBeginCommandBuffer(cmd, &bi) != VK_SUCCESS) goto out;
			VkClearValue clear = {{{0, 0, 0, 1}}};
			VkRenderPassBeginInfo rpbi = {
				.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
				.renderPass = rp,
				.framebuffer = fb,
				.renderArea = {{0, 0}, {16, 16}},
				.clearValueCount = 1,
				.pClearValues = &clear,
			};
			pCmdBeginRenderPass(cmd, &rpbi, VK_SUBPASS_CONTENTS_INLINE);
			pCmdEndRenderPass(cmd);
			if (pEndCommandBuffer(cmd) != VK_SUCCESS) goto out;
			VkSubmitInfo si = {
...
out:
	if (pool) pDestroyCommandPool(dev, pool, NULL);
	if (fb) pDestroyFramebuffer(dev, fb, NULL);
	if (rp) pDestroyRenderPass(dev, rp, NULL);
	if (view) pDestroyImageView(dev, view, NULL);
	if (img) pDestroyImage(dev, img, NULL);
	if (mem) pFreeMemory(dev, mem, NULL);
	if (done) LOGI("fresh-device warm-up done (%d rounds)", WARMUP_ROUNDS);
...
static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL shim_GetInstanceProcAddr(
	...
	if (!strcmp(name, "vkCreateDevice")) {
		// Resolve everything the fresh-device warm-up needs while an instance
		// handle is available; the CreateDevice hook itself only gets the
		// physical device.
		if (!real_cdv) real_cdv = (PFN_vkCreateDevice)real_gipa(inst, name);
		if (!real_gdpa)
			real_gdpa = (PFN_vkGetDeviceProcAddr)real_gipa(inst, "vkGetDeviceProcAddr");
		if (!real_gpdmp)
			real_gpdmp = (PFN_vkGetPhysicalDeviceMemoryProperties)
				real_gipa(inst, "vkGetPhysicalDeviceMemoryProperties");
		return real_cdv ? (PFN_vkVoidFunction)shim_CreateDevice : NULL;
	}
	if (!strcmp(name, "vkCreateGraphicsPipelines")) {
