...
// libtz_uree.so was restored to /vendor — TEE spike; gatekeeper
// spike proved it enrolls+verifies with a 69-byte HardwareAuthToken and
// needs NO libgatekeeper_mtee.so, its only TEE dep being libtz_uree).
//
// Why this fixes the PIN regression: the gatekeeper trustlet and the keymaster
// trustlet share the TEE root of trust, so the HardwareAuthToken this HAL signs
// is trusted by the HW keymaster. With the software gatekeeper, keymaster
...
//
// Defensive fallback (mirrors the keystore tryGetService guard): if the
