...
Gatekeeper::Gatekeeper()
{
    ...
    // xdplus: do NOT LOG_ALWAYS_FATAL on failure. If the trustlet HAL can't open
    // (missing libtz_uree, TEE unreachable), leave device==NULL so the service
    // main can fall back to the software gatekeeper instead of crash-looping.
    if (ret < 0 || device == NULL) {
        ALOGE("Unable to open TEE GateKeeper HAL (ret=%d) — will fall back", ret);
        device = NULL;
    }
}
